...
from nebula import PEDynamicFeatureExtractor, JSONTokenizerNaive

# SCRIPT CONFIG

OUTFOLDER_SUFFIX = "_WHITESPACE_50k"
...
def main(limit=None, mode="readAndFilter"):
    ...
    for vocabSize in VOCAB_SIZES:
        for maxSeqLen in MAX_SEQ_LENGTHS: 
            ...
            filePrefix = f"speakeasy_vocab_size_{vocabSize}"
            ...
            tokenizer.train(eventsTokenizedTrain, vocab_size=vocabSize,
                            model_prefix=os.path.join(trainOutFolder, f"{filePrefix}_tokenizer"),
            ...
            eventsEncodedTrain = tokenizer.encode(eventsTokenizedTrain, pad=True)
            eventsEncodedTest = tokenizer.encode(eventsTokenizedTest, pad=True)
            ...
            np.save(os.path.join(trainOutFolder, f"{filePrefix}_maxlen_{maxSeqLen}_x.npy"), eventsEncodedTrain)
            np.save(os.path.join(testOutFolder, f"{filePrefix}_maxlen_{maxSeqLen}_x.npy"), eventsEncodedTest)
...
if __name__ == "__main__":
    ...
    outputFolder = os.path.join(REPO_ROOT, "data", "data_filtered", "speakeasy_parsing_logs")
